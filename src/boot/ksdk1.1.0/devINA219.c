	/*
	made using devMMA8451Q.c as a template
	...
	*/
	...
	{
		uint8_t		payloadByte[1], commandByte[1];  // need to set the calibration register to be not 0? if we want current and power registers, otherwise can leave as 0
		i2c_status_t	status;

		switch (deviceRegister)
		{
			case 0x09: case 0x0a: case 0x0e: case 0x0f:   //I think this bit is not needed as there are only 5 registers?
	...
	configureSensorINA219(uint8_t payloadF_SETUP, uint8_t payloadCTRL_REG1)
	{
		...
		i2cWriteStatus1 = writeSensorRegisterINA219(kWarpSensorConfigurationRegisterINA219Config /* register address F_SETUP */,
													payloadF_SETUP /* payload: Disable FIFO */
		);

		i2cWriteStatus2 = writeSensorRegisterINA219(kWarpSensorConfigurationRegisterINA219Calib /* register address CTRL_REG1 */,
